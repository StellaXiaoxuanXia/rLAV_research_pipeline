...
ncores <- 1
cat(sprintf("[%s] Initializing permutation run... Using %d core for LASSO regression\n", Sys.time(), ncores))
...
option_list <- list(
  ...
  make_option(c("--n_perms"), type = "integer", default = 10),
...
lav_bi_rds <- file.path(rds_dir, "LAV.VNTR.biallelic.rds")
nlav_rlav_bi_rds <- file.path(rds_dir, "nLAV+rLAV.VNTR.rds")
lav_cn_rds <- file.path(rds_dir, "LAV.VNTR.copynumber.rds")
nlav_rlav_bi_cn_rds <- file.path(rds_dir, "nLAV+rLAV.VNTR.biallelic+copynumber.rds")
advntr_rds <- file.path(rds_dir, "adVNTR.QCed.rds")
vntrseek_rds <- file.path(rds_dir, "VNTRseek.QCed.rds")
...
cat("Split directory: ", split_dir, "\n", sep = "")
cat("Permutation phenotype root: ", opt$perm_root, "\n", sep = "")
cat("Permutation range: ", opt$perm_start, "-", opt$perm_start + opt$n_perms - 1L, "\n", sep = "")
...
advntr <- load_obj(advntr_rds)
vntrseek <- load_obj(vntrseek_rds)
...
cols <- list(
  ...
  adVNTR = seq_len(ncol(advntr$genotypes)),
  VNTRseek = seq_len(ncol(vntrseek$genotypes))
)
...
feature_sets <- list(
  ...
  adVNTR = list(obj = advntr, cols = cols$adVNTR),
  VNTRseek = list(obj = vntrseek, cols = cols$VNTRseek)
)
...
run_one_permutation <- function(perm_idx) {
  ...
  perm_out_dir <- file.path(out_root, sprintf("perm_%d", perm_idx))
  ...
  fwrite(r2_wide, file.path(perm_out_dir, sprintf("trait%s_model_r2.tsv", trait)), sep = "\t")

  cat("Saved permutation results in ", perm_out_dir, "\n", sep = "")
}

perm_indices <- seq.int(opt$perm_start, opt$perm_start + opt$n_perms - 1L)
for (perm_idx in perm_indices) {
  run_one_permutation(perm_idx)
}

cat("Finished permuted prediction. Results saved in ", out_root, "\n", sep = "")
