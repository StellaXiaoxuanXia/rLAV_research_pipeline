...
results <- mclapply(genes, function(gene) {
  message(sprintf("Processing QCed STR: %s", gene))

  cmd <- sprintf(
    "%s prediction_pipeline.STR.QCed.wg.final.R --split_dir splits_10rep --n_reps 10 --trait %s --out_dir results/STR/10rep_QCed",
    shQuote(rscript),
    shQuote(gene)
  )
