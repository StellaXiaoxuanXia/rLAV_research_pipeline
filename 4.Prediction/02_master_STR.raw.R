...
results <- mclapply(genes, function(gene) {
  message(sprintf("Processing raw STR: %s", gene))

  cmd <- sprintf(
    "%s prediction_pipeline.STR.raw.wg.final.R --split_dir splits_10rep --n_reps 10 --trait %s --out_dir results/STR/10rep_raw",
    shQuote(rscript),
    shQuote(gene)
  )
