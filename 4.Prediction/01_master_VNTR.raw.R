...
results <- mclapply(genes, function(gene) {
  message(sprintf("Processing raw VNTR: %s", gene))

  cmd <- sprintf(
    "%s prediction_pipeline.VNTR.raw.wg.final.R --split_dir splits_10rep --n_reps 10 --trait %s --out_dir results/VNTR/10rep_raw",
    shQuote(rscript),
    shQuote(gene)
  )
