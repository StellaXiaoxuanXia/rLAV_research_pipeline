...
This script processes 30 synchronized permutation replicates in an internal loop.
It includes I/O retry handling and checkpoint-aware resume support.

Updated version:
    - Only runs three specified TRs_raw GRMs:
        1. TRs_STR
        2. TRs_adVNTR
        3. TRs_VNTRseek
"""
...
# 2. GRM Registry
# ==========================================
# This updated version only evaluates three TRs_raw GRMs.
GRM_CONFIGS = {
    "TRs_STR": f"{MAIN_DIR}/03_WG_hsq_comparison_ManualGRM_TRs_raw/GRMs/grm_manual_STR",
    "TRs_adVNTR": f"{MAIN_DIR}/03_WG_hsq_comparison_ManualGRM_TRs_raw/GRMs/grm_manual_adVNTR",
    "TRs_VNTRseek": f"{MAIN_DIR}/03_WG_hsq_comparison_ManualGRM_TRs_raw/GRMs/grm_manual_VNTRseek"
}

print("[Init] Validating 3 GRMs...", flush=True)
...
for rep_id in range(1, TOTAL_REPS + 1):
    ...
    print("=" * 60)
    print(f"[Start] Synchronized permutation replicate {rep_id:02d} / {TOTAL_REPS}")
    print("=" * 60, flush=True)
    ...
    # Use a distinct output filename to avoid confusion with the full-GRM version.
    SUMMARY_FILE = f"{WORK_DIR}/summary_Synced_Permutation_TRs_raw_rep{rep_id:02d}.tsv"
    ...
    # Checkpoint-aware resume support
    ...
    if os.path.exists(SUMMARY_FILE):
        try:
            # Read completed phenotype records from the existing summary file.
        ...
        # First run: initialize the summary file with a header.
        os.makedirs(os.path.dirname(SUMMARY_FILE), exist_ok=True)

        with open(SUMMARY_FILE, "w") as f:
            f.write("Phenotype\tModel\tVG_Vp\tSE\tPval\n")

    # Initialize the RAM-disk working directory.
    ...
    # Run only the pending phenotypes for the current replicate.
    ...
    try:
        with ThreadPoolExecutor(max_workers=WORKER_THREADS) as executor:
            ...
            for future in as_completed(future_to_pheno):
                ...
                # I/O retry handling for unstable cluster file systems.
                write_success = False

                for attempt in range(10):
                    try:
                        with write_lock:
                            # Defensive directory recreation in case the NFS path becomes temporarily unavailable.
                            os.makedirs(os.path.dirname(SUMMARY_FILE), exist_ok=True)
        ...
        # Release RAM-disk storage for the current replicate.
...
print("=" * 60)
print(f"[Done] All {TOTAL_REPS} permutations finished successfully.")
