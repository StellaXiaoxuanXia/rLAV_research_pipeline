"""
Process copy-number TSV datasets: LAV_STR and LAV_VNTR.

This script parses TSV-formatted copy-number matrices, performs manual
imputation and standardization, constructs genetic relationship matrices
(GRMs) directly from the processed matrices, and saves the resulting GRM
files for downstream heritability estimation.
"""
...
WORK_DIR = f"{MAIN_DIR}/04_WG_hsq_comparison_ManualGRM_LAV_CN"
...
BASE_MODELS = {
    "LAV_STR": f"{TR_DIR}/LAV_STR.copy_number_exact.tsv",
    "LAV_VNTR": f"{TR_DIR}/LAV_VNTR.copy_number_exact.tsv"
}

TARGET_MODELS = ["LAV_STR", "LAV_VNTR"]
...
def load_and_impute_tsv(filepath, valid_samples):
    """
    Parse and transpose TSV files where variants are stored as rows and
    samples are stored as columns, with the first metadata columns describing
    ...
    if dropped_cols > 0:
        print(f"        [Clean] Dropped {dropped_cols} variant columns that were entirely NA.")
...
print("=== Starting Copy-Number GRM Construction ===", flush=True)
...
# ==========================================
print("[Prep] Manually calculating GRMs directly from TSV copy-number matrices...", flush=True)
...
print(f"\n[Done] Copy-number GRMs have been generated and saved to: {GRM_OUT_DIR}")
