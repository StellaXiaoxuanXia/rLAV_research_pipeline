"""
Genome-wide STR heritability simulation using prebuilt GRMs and mixed causal
features sampled from a pruned feature pool.

Key design:
    - Sample causal features only from the feature space used to construct the
      joint model: copy-number features plus pruned rLAV+nLAV sequence features.
    - Allocate the fixed number of causal features between copy-number and
      sequence features according to their relative feature counts.
    - Evaluate whether the joint GRM is approximately unbiased when the causal
      feature space and tested feature space are fully aligned.
"""
...
POOL_BFILE_RLAV = f"{BFILE_DIR}/rLAV_STR"
POOL_BFILE_NLAV = f"{BFILE_DIR}/nLAV_STR"
TSV_COPY_NUMBER = f"{BASE}/06_dosage/LAV_STR.copy_number_exact.tsv"

# Pruned variant ID list used by the joint GRM.
WORK_DIR = f"{BASE}/25_hsq_real_data/08_Pruned_GRM_Inputs"
KEEP_LIST = f"{WORK_DIR}/pruned_rLAV_nLAV_STR_r2_0.05.keep.snplist"

# Output directory for mixed-causal simulations sampled from the pruned pool.
OUT_DIR = f"{BASE}/24_hsq_simulation/real_data_simulation/results_STR_prebuilt_GRMs_Mixed_causal_pruned_pool_100reps"

PREBUILT_GRMS = {
    "CN": f"{BASE}/25_hsq_real_data/04_WG_hsq_comparison_ManualGRM_LAV_CN/GRMs/grm_manual_LAV_STR",
    "rLAV": f"{BASE}/25_hsq_real_data/01_WG_hsq_comparison_ManualGRM_raw_STR/GRMs/grm_manual_rLAV",
    "LAV": f"{BASE}/25_hsq_real_data/01_WG_hsq_comparison_ManualGRM_raw_STR/GRMs/grm_manual_LAV",
    "rLAV_nLAV": f"{BASE}/25_hsq_real_data/01_WG_hsq_comparison_ManualGRM_raw_STR/GRMs/grm_manual_rLAV_nLAV",
    "Joint_Pruned_05": f"{BASE}/25_hsq_real_data/08_Pruned_GRM_Inputs/GRMs/grm_joint_pruned_CN_rLAV_nLAV_STR_r2_0.05"
}
...
def main():
    log.info(
        f"Starting STR mixed-causal simulation using the pruned joint feature pool. "
        f"Threads: {THREADS}"
    )

    # 1. Parse the anchor sample list.
    log.info(f"Phase 1: Reading anchor sample list from {PREBUILT_GRMS['CN']}.grm.id ...")
    if not os.path.exists(PREBUILT_GRMS["CN"] + ".grm.id"):
        log.error(f"Missing base GRM ID file: {PREBUILT_GRMS['CN']}.grm.id")
        sys.exit(1)

    df_id = pd.read_csv(
        PREBUILT_GRMS["CN"] + ".grm.id",
        sep="\t",
        header=None,
        names=["FID", "IID"]
    )
    ...
    log.info(f"Aligned samples: {N_samples}")

    # 2. Load the full copy-number matrix and count copy-number features.
    log.info("Phase 2: Loading copy-number data and counting CN features...")
    ...
    # 3. Build the strict pruned sequence-variant sampling pool.
    log.info("Phase 3: Reading the pruned variant list to build the sequence-feature pool...")
    ...
    # 4. Dynamically allocate causal features by feature-space proportion.
    ...
    log.info("=" * 60)
    log.info("Dynamic proportional sampling configuration for pruned features only:")
    log.info(f"   -> Copy-number features: {N_loci_cn:,}")
    log.info(f"   -> rLAV+nLAV features after pruning at r2 < 0.05: {pool_size:,}")
    log.info(f"   -> Target total causal features: {N_CAUSAL}")
    log.info(
        f"   -> Allocation: {n_causal_cn} causal features from CN; "
        f"{n_causal_seq} causal features from pruned sequence features"
    )
    log.info("=" * 60)

    # 5. Run simulation replicates.
    log.info("Phase 4: Sampling mixed causal features, simulating phenotypes, and queuing REML jobs...")
    reml_tasks = []
    rng = np.random.default_rng(SEED)
    tmp_dir = os.path.join(OUT_DIR, "tmp")

    for rep in range(1, N_REPS + 1):
        # A. Sample copy-number causal features.
        cn_idx = (
            rng.choice(N_loci_cn, n_causal_cn, replace=False)
            if N_loci_cn >= n_causal_cn
            else np.arange(N_loci_cn)
        )
        X_causal_cn = X_cn_full[:, cn_idx]

        # B. Sample sequence causal features from the pruned variant list only.
        seq_idx = (
            rng.choice(pool_size, n_causal_seq, replace=False)
            if pool_size >= n_causal_seq
        ...
        # Force the sequence matrix to match the anchor sample order.
        ...
        # C. Combine CN and sequence causal features and construct the oracle causal GRM.
        ...
        # Remove temporary PLINK files for the current replicate.
        for f in glob.glob(f"{tmp_dir}/*_{rep}*"):
            os.remove(f)

        # D. Simulate phenotypes from the mixed causal feature set.
    ...
    # 6. Run REML jobs concurrently.
    log.info(f"Phase 5: Running {len(reml_tasks)} REML models concurrently...")
    all_results = []
    n_done = 0
    ...
    # 7. Export result tables and summary statistics.
    ...
    if not df.empty:
        ...
        log.info("\n" + "=" * 85)
        log.info("Simulation summary with aligned feature space: CN + pruned sequence features")
        log.info("=" * 85)
        ...
        log.info(f"Analysis completed. Summary saved to: {OUT_DIR}/summary_simulation_bias.tsv")
